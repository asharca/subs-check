...
type Config struct {
	...
	PlatformTags         map[string]string `yaml:"platform-tags"`
...
// GetPlatformTag 获取平台标签，优先使用自定义标签，否则使用默认标签
func GetPlatformTag(platform string, defaultTag string) string {
	if GlobalConfig.PlatformTags != nil {
		if customTag, ok := GlobalConfig.PlatformTags[platform]; ok && customTag != "" {
			return customTag
		}
	}
	return defaultTag
